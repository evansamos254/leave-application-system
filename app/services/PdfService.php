...
class PdfService
{
    ...
    private function officialLeaveForm(array $request, array $steps): void
    {
        ...
        $reportedBack = !empty($request['resumed_at'])
            ? format_date($request['resumed_at']) . (!empty($request['resumed_by_name']) ? ' by ' . $request['resumed_by_name'] : '')
            : 'Not yet recorded';

        $this->pageFrame();
        $this->headerBlock((int) $request['id'], $approved);

        $this->sectionBand(700, 'PART A: EMPLOYEE DETAILS');
        ...
        $this->fieldBox(50, 42, 248, 30, 'Expected report-back date', $approved ? format_date($reportBackDate) : '-');
        $this->fieldBox(298, 42, 247, 30, 'Reported back', $approved ? $reportedBack : '-');

        if ($approved) {
            $this->stamp(436, 726, 'APPROVED');
        }
    ...
    private function headerBlock(int $requestId, bool $approved): void
    {
        ...
        $this->textAt(438, 768, 'Status: ' . ($approved ? 'Approved' : 'In progress'), 8);
        $this->colorLine(50, 724, 495, 3);
    }
    ...
    private function drawImage(float $x, float $y, float $width): void
    {
        $command = $this->imageCommand($x, $y, $width);
        if ($command !== '') {
            $this->commands[] = $command;
        }
    }

    private function imageCommand(float $x, float $y, float $width): string
    {
        ...
        return sprintf('q %.2F 0 0 %.2F %.2F %.2F cm /WmLogo Do Q', $width, $height, $x, $y);
    }
    ...
    private function build(array $pageStreams): string
    {
        ...
        if ($this->watermark !== null) {
            ...
            $objects[$watermarkImageId] = "<< /Type /XObject /Subtype /Image /Width "
                . (int) $this->watermark['width']
                . " /Height "
                . (int) $this->watermark['height']
                . " /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length "
                . strlen($this->watermark['data'])
                . " >>\nstream\n"
                . $this->watermark['data']
                . "\nendstream";
        ...
        foreach ($pageStreams as $stream) {
            ...
            if ($watermarkImageId !== null && $watermarkStateId !== null) {
                $resources .= ' /XObject << /WmLogo ' . $watermarkImageId . ' 0 R >>'
                    . ' /ExtGState << /WmState ' . $watermarkStateId . ' 0 R >>';
            }
